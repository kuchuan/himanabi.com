            ...
            <div class="col-sm-10 my-md-5 rounded p-3 shadow-sm" style="background-color:#FFFFCC"><!--中央パート-->
                ...
                <div class="col-5"><!--中央パートの左側-->
                            <div class="form-group text-left"><!--カテゴリ-->
                                ...
                                <select class="form-control" name="skills_category" id="skills_category" value="{{$datas->skills->first()->skills_category}}">
                                    <option {{ ($datas->skills->first()->skills_category)==="語学" ? 'selected' : ''}}>語学</option>
                                    <option {{ ($datas->skills->first()->skills_category)==="教育" ? 'selected' : ''}}>教育</option>
                                ...
                                <div class="form-group text-left">
                                    ...
                                    <input type="text" class="form-control" name="skills_name" id="skills_name" value="{{ $datas->skills->first()->skills_name }}" placeholder="スキルの名前を入力してください ">
                                </div>

                                <div class="form-group text-left">
                                    <label class="small text-info" for="textArea1">希望期間</label>
                                        <div class="input-group mb-3">
                                          <input type="number" class="form-control" placeholder="期間を記入してください" aria-label="希望期間" aria-describedby="skills_get" value="{{ $datas->skills->first()->skills_get }}">
                          ...
                          <div class="form-group text-left">
                            ...
                            <textarea class="form-control" name="skills_enthusiasm" id="skills_enthusiasm" rows="3">{{ $datas->skills->first()->skills_enthusiasm }}</textarea>
                          </div>
                </div>{{-- 中央パートの左側 --}}
                ...
                <div class="col-5"><!--中央パートの右側-->
                            <div class="form-group text-left" style="align-self:flex-end">
                                ...
                                <select class="form-control"name="skills_area" id="skills_area" value="{{ $datas->skills->first()->skills_area }}">
                                    <option {{ ($datas->skills->first()->skills_area)==="ラフグ" ? 'selected' : ''}}>ラフグ</option>
                                    <option {{ ($datas->skills->first()->skills_area)==="マンダウエ" ? 'selected' : ''}}>マンダウエ</option>
                                    <option {{ ($datas->skills->first()->skills_area)==="バニラッド" ? 'selected' : ''}}>バニラッド</option>
                                    <option {{ ($datas->skills->first()->skills_area)==="カサンバガン" ? 'selected' : ''}}>カサンバガン</option>
                                    <option {{ ($datas->skills->first()->skills_area)==="マクタン" ? 'selected' : ''}}>マクタン</option>
                                    <option {{ ($datas->skills->first()->skills_area)==="イロイロ" ? 'selected' : ''}}>イロイロ</option>
                                    <option {{ ($datas->skills->first()->skills_area)==="その他" ? 'selected' : ''}}>その他</option><option>らふぐ</option>
                                </select>
                            </div>
                </div>{{-- ./中央パートの右側 --}}
                                  ...
                                  <div class="checkbox">
                                    <div class="form-row" id="checkBoxies1">
                                            <div class="form-check form-check-inline small ml-2">
                                                <label class="checkbox-inline ml-1"><input type="checkbox" value="" name="interest01" id="interest01" value="0" {{ ($datas->skills->first()->interest01) ? 'checked="checked"' : ''}}>：本・コミック・雑誌　</label>
                                                <label class="checkbox-inline ml-1"><input type="checkbox" value="" name="interest02" id="interest02" value="0" {{ ($datas->skills->first()->interest02) ? 'checked="checked"' : ''}}>：芸能・アニメ　　　　</label>
                                                <label class="checkbox-inline ml-1"><input type="checkbox" value="" name="interest03" id="interest03" value="0" {{ ($datas->skills->first()->interest03) ? 'checked="checked"' : ''}}>：ミュージック　　　　</label>
                                                <label class="checkbox-inline ml-1"><input type="checkbox" value="" name="interest04" id="interest04" value="0" {{ ($datas->skills->first()->interest04) ? 'checked="checked"' : ''}}>：家電・カメラ・AV機器</label>
                                            </div>
                                    </div>
                                    <hr>
                                    <div class="form-row" id="checkBoxies2">
                                            <div class="form-check form-check-inline small ml-2">
                                                <label class="checkbox-inline ml-1"><input type="checkbox" value="" name="interest05" id="interest05" value="0" {{ ($datas->skills->first()->interest05) ? 'checked="checked"' : ''}}>：ＰＣ・ガジェット　　</label>
                                                <label class="checkbox-inline ml-1"><input type="checkbox" value="" name="interest06" id="interest06" value="0" {{ ($datas->skills->first()->interest06) ? 'checked="checked"' : ''}}>：ホーム＆キッチン　　</label>
                                                <label class="checkbox-inline ml-1"><input type="checkbox" value="" name="interest07" id="interest07" value="0" {{ ($datas->skills->first()->interest07) ? 'checked="checked"' : ''}}>：美容・健康　　　　　</label>
                                                <label class="checkbox-inline ml-1"><input type="checkbox" value="" name="interest08" id="interest08" value="0" {{ ($datas->skills->first()->interest08) ? 'checked="checked"' : ''}}>：ファッション・貴金属</label>
                                            </div>
                                    </div>
                                    <hr>
                                    <div class="form-row" id="checkBoxies3">
                                            <div class="form-check form-check-inline small ml-2">
                                                <label class="checkbox-inline ml-1"><input type="checkbox" value="" name="interest09" id="interest09" value="0" {{ ($datas->skills->first()->interest09) ? 'checked="checked"' : ''}}>：子育て・介護　　　　</label>
                                                <label class="checkbox-inline ml-1"><input type="checkbox" value="" name="interest10" id="interest10" value="0" {{ ($datas->skills->first()->interest10) ? 'checked="checked"' : ''}}>：動物・ペット　　　　</label>
                                                <label class="checkbox-inline ml-1"><input type="checkbox" value="" name="interest11" id="interest11" value="0" {{ ($datas->skills->first()->interest11) ? 'checked="checked"' : ''}}>：仕事・オフィス　　　</label>
                                                <label class="checkbox-inline ml-1"><input type="checkbox" value="" name="interest12" id="interest12" value="0" {{ ($datas->skills->first()->interest12) ? 'checked="checked"' : ''}}>：スポーツ・アウトドア</label>
                                            </div>
                                    </div>
                                    <hr>
                                    <div class="form-row" id="checkBoxies4">
                                            <div class="form-check form-check-inline small ml-2">
                                                <label class="checkbox-inline ml-1"><input type="checkbox" value="" name="interest13" id="interest13" value="0" {{ ($datas->skills->first()->interest13) ? 'checked="checked"' : ''}}>：芸術・アート　　　　</label>
                                                <label class="checkbox-inline ml-1"><input type="checkbox" value="" name="interest14" id="interest14" value="0" {{ ($datas->skills->first()->interest14) ? 'checked="checked"' : ''}}>：食品・飲料・酒　　　</label>
                                                <label class="checkbox-inline ml-1"><input type="checkbox" value="" name="interest15" id="interest15" value="0" {{ ($datas->skills->first()->interest15) ? 'checked="checked"' : ''}}>：金融・ファイナンス　</label>
                                                <label class="checkbox-inline ml-1"><input type="checkbox" value="" name="interest16" id="interest16" value="0" {{ ($datas->skills->first()->interest16) ? 'checked="checked"' : ''}}>：おもちゃ・ゲーム　　</label>
                                            </div>
                                    </div>
                                    <hr>
                                    <div class="form-row" id="checkBoxies5">
                                            <div class="form-check form-check-inline small ml-2">
                                                <label class="checkbox-inline ml-1"><input type="checkbox" value="" name="interest17" id="interest17" value="0" {{ ($datas->skills->first()->interest17) ? 'checked="checked"' : ''}}>：自動車・バイク・船　</label>
                                                <label class="checkbox-inline ml-1"><input type="checkbox" value="" name="interest18" id="interest18" value="0" {{ ($datas->skills->first()->interest18) ? 'checked="checked"' : ''}}>：ＤＩＹ・ハンドメイド</label>
                                            </div>
                                    </div>
